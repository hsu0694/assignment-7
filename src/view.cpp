...
void View::updateGameObject(GameObject* obj,GameObject* _player) {
    Icon icon = obj->getIcon();
        ...
        for (std::size_t dy = 0; dy < icon.size(); ++dy) {
            ...
            for (std::size_t dx = 0; dx < icon[dy].size(); ++dx) {
				...
				std::string txt = cell.ascii;
				if (obj == _player && icon[dy].size() >= 1) {
					if (dx == 0) txt = "[" + txt ;
					if (dx == icon[dy].size() - 1) txt = txt + "]";
				}
				latest_map[row][col] = txt;
